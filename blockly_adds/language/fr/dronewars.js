Blockly.LANG_CATEGORY_DRONEWARS = 'DroneWars';
Blockly.LANG_CATEGORY_DRONEWARS_COORD = 'coordonées';
Blockly.LANG_CATEGORY_DRONEWARS_RANDDIR = 'direction aléatoire';
Blockly.LANG_CATEGORY_DRONEWARS_DIR_FROM_TO_1 = 'direction de';
Blockly.LANG_CATEGORY_DRONEWARS_DIR_FROM_TO_2 = 'vers';
Blockly.LANG_CATEGORY_DRONEWARS_MAKE_BUILDING = 'ordre de créer un batiment ';
Blockly.LANG_CATEGORY_DRONEWARS_MAKE_SHIP = 'ordre de créer un vaisseau ';
Blockly.LANG_CATEGORY_DRONEWARS_MAKE_CANNON = 'ordre de créer un canon';
Blockly.LANG_CATEGORY_DRONEWARS_PLANET_DO_NOTHING = 'ordre de ne rien faire';
Blockly.LANG_CATEGORY_DRONEWARS_IS_PLANET_FREE_1 = 'la planète';
Blockly.LANG_CATEGORY_DRONEWARS_IS_PLANET_FREE_2 = 'est libre';
Blockly.LANG_CATEGORY_DRONEWARS_PLANET_COORD = 'coordonées de la planète';
Blockly.LANG_CATEGORY_DRONEWARS_PLANET_PLAYERID = "code de l'habitant de la planète";
Blockly.LANG_CATEGORY_DRONEWARS_PLANET_BUILDINGLIST = 'batiments de la planète';
Blockly.LANG_CATEGORY_DRONEWARS_PLANET_CANNONLIST = 'canons de la planète';
Blockly.LANG_CATEGORY_DRONEWARS_PLANET_RESSOURCESET = 'resources de la planète';
Blockly.LANG_CATEGORY_DRONEWARS_RESSOURCESET = 'resources';
Blockly.LANG_CATEGORY_DRONEWARS_RESSOURCESET_AT = ' présent dans les ressources';
Blockly.LANG_CATEGORY_DRONEWARS_BUILDING_IN_PLANET = ' présent sur la planète ';
Blockly.LANG_CATEGORY_DRONEWARS_CANNON_IN_PLANET = Blockly.LANG_CATEGORY_DRONEWARS_BUILDING_IN_PLANET;
Blockly.LANG_CATEGORY_DRONEWARS_SHIP_IN_FLEET = ' présent dans la flotte ';

Blockly.LANG_CATEGORY_DRONEWARS_FLEET_ID = 'code de la flotte';
Blockly.LANG_CATEGORY_DRONEWARS_FLEET_PLAYER_ID = 'code du propriétaire de la flotte';
Blockly.LANG_CATEGORY_DRONEWARS_FLEET_COORD = 'coordonées de la flotte';
Blockly.LANG_CATEGORY_DRONEWARS_FLEET_ORIGIN = 'origine de la flotte';
Blockly.LANG_CATEGORY_DRONEWARS_FLEET_NAME = 'nom de la flotte';
Blockly.LANG_CATEGORY_DRONEWARS_FLEET_SHIPLIST = 'vaisseaux de la flotte';
Blockly.LANG_CATEGORY_DRONEWARS_FLEET_RESSOURCESET = 'resources de la flotte';

Blockly.LANG_CATEGORY_DRONEWARS_FLEETACTION_CTOR_1 = 'ordre de faire';
Blockly.LANG_CATEGORY_DRONEWARS_FLEETACTION_CTOR_2 = ' dans la direction';


Blockly.LANG_DRONEWARS_SHIP1 = 'Mosquito';
Blockly.LANG_DRONEWARS_SHIP2 = 'Hornet';
Blockly.LANG_DRONEWARS_SHIP3 = 'Vulture';
Blockly.LANG_DRONEWARS_SHIP4 = 'Dragon';
Blockly.LANG_DRONEWARS_SHIP5 = 'Behemoth';
Blockly.LANG_DRONEWARS_SHIP6 = 'Azathoth';
Blockly.LANG_DRONEWARS_SHIP7 = 'Queen';
Blockly.LANG_DRONEWARS_SHIP8 = 'Cargo';
Blockly.LANG_DRONEWARS_SHIP9 = 'Grand cargo';

Blockly.LANG_DRONEWARS_CANNON1 = 'Canon1';
Blockly.LANG_DRONEWARS_CANNON2 = 'Canon2';
Blockly.LANG_DRONEWARS_CANNON3 = 'Canon3';
Blockly.LANG_DRONEWARS_CANNON4 = 'Canon4';
Blockly.LANG_DRONEWARS_CANNON5 = 'Canon5';
Blockly.LANG_DRONEWARS_CANNON6 = 'Canon6';

Blockly.LANG_DRONEWARS_FLEETACTION1 = 'Rien';
Blockly.LANG_DRONEWARS_FLEETACTION2 = 'Voyage';
Blockly.LANG_DRONEWARS_FLEETACTION3 = 'Récolte';
Blockly.LANG_DRONEWARS_FLEETACTION4 = 'Colonise';
Blockly.LANG_DRONEWARS_FLEETACTION5 = 'Décharge';

Blockly.LANG_DRONEWARS_BUILDING1 = 'Centre de commande';
Blockly.LANG_DRONEWARS_BUILDING2 = 'Mine de métal';
Blockly.LANG_DRONEWARS_BUILDING3 = 'Mine carbonique';
Blockly.LANG_DRONEWARS_BUILDING4 = 'Filtre a Loïcium';
Blockly.LANG_DRONEWARS_BUILDING5 = 'Fabrique';
Blockly.LANG_DRONEWARS_BUILDING6 = 'Laboratoire';
Blockly.LANG_DRONEWARS_BUILDING7 = 'Centrale carbonique';
Blockly.LANG_DRONEWARS_BUILDING8 = 'Central solaire';
Blockly.LANG_DRONEWARS_BUILDING9 = 'Central géothermique';
